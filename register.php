...
if (IS_POST) {
    ...
    if (!input_check('user_name', $name, $error)) {
        register_display("注册失败，用户名格式有误，要求：$error");
    }
    if (!input_check('user_password', $password, $error)) {
        register_display("注册失败，密码格式有误，要求：$error");
    }
    if (!input_check('user_email', $email)) {
        register_display('注册失败，邮箱格式有误。');
    }
    if (!register($name, $password, $email, $error)) {
        register_display("注册失败，$error", $name, $email);
    }
}
// 显示页面
register_display();
...
// 显示页面并退出
function register_display($tips = null, $name = '', $email = '')
{
